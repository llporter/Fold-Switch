...
int main(int argc, char *argv[])
{
	...
	cout << "Done with retrieveAnnotations" << endl;
			...
			{	getline(infile3,line3);
				...
				if (line3.substr(0,9) == "Template:")
				{
					templateRead = line3.substr(10);
				}
				else if (line3[0] == '>')
				{
					...
					string currSS = line3;
					getline(infile3,line3);
					string currBLAST = line3;
					
					if (perfectMatch(currAlign))
					{	outfile3 << currName << endl << replacePeriods(currAlign,templateRead) << endl << currSS << endl << currBLAST << endl;
					}
				}
				else
					outfile3 << line3 << endl;
				...
				{	int numSeqs = ss.size();
					...
					for (int i = 0; i < numSeqs - 1; i++)
					{
						...
						for (int j = i+1; j < numSeqs; j++)
						{
							...
							if (hammDist > minHammDist && hammDiff > minHammDiff && nonBlankResidues > minGraftSize)
							{	outfile4 << names[i] << " & " << names[j] << " might fold switch:" << endl;
								outfile4 << BLASTs[i] << endl << BLASTs[j] << endl;
								outfile4 << aligns[i] << endl << aligns[j] << endl;
								outfile4 << SSi << endl << SSj << endl;
								outfile4 << "hammDist = " << hammDist << ", hammDiff = " << hammDiff << endl << endl;
							}
						}
					}
